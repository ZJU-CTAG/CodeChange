...
from compute_gleu import calcGleu
    ...
    if args.task in ['defect']:
        ...
        with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
            for pred_nl, gold in zip(pred_nls, eval_examples):
                ...
                if args.task in ['summarize']:
                    ...
                    f.write(pred_nl.replace('\n','').strip() + '\n')
                    f1.write(gold.target.strip() + '\n')
                    f2.write(gold.diff.strip() + '\n')
        ...
        if args.task == 'summarize':
            ...
            for pred, gold in zip(pred_nls, eval_examples):
                src_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(gold.ori_comment)])
                gold_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(gold.target)])
                pred_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(pred)])
            gleu = calcGleu(src_tokens, gold_tokens, pred_tokens)
...
def eval_gleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    logger.info("  ***** Running bleu evaluation on {} data*****".format(split_tag))
    logger.info("  Num examples = %d", len(eval_examples))
    logger.info("  Batch size = %d", args.eval_batch_size)
    eval_sampler = SequentialSampler(eval_data)
    ...
    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
        for pred_nl, gold in zip(pred_nls, eval_examples):
            ...
            if args.task in ['summarize']:
                ...
                f1.write(gold.target.strip() + '\n')
                f2.write(gold.diff.strip() + '\n')
                src_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(gold.diff.strip())])
                gold_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(gold.target.strip())])
                pred_tokens.append([x.lower().replace("``", "\"") for x in tokenizer.tokenize(pred_nl.strip())])
...
def main():
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size,
                                      num_workers=4, pin_memory=True)
        print('src:', tokenizer.decode(list(train_data[0][0]), skip_special_tokens=True))
        print('tgt:', tokenizer.decode(list(train_data[0][1]), skip_special_tokens=True))
    ...
    if args.do_test:
        ...
        for criteria in ['last','best-ppl']:
            ...
            test_gleu = result['gleu'] if 'gleu' in result else 0
            result_str = "[%s] bleu-4: %.2f, em: %.4f, codebleu: %.4f, gleu: %.4f\n" % (criteria, test_bleu, test_em, test_codebleu,test_gleu)
