...
class DQEClsDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        self.tokenizer = tokenizer
        examples = []
        logger.info("Loading examples from {}".format(file_path))
        if file_path.endswith('pkl'):
            data = pickle.load(open(file_path, "rb"))
            examples = read_review_pkl_examples(args, data, samplenum, tokenizer)
...
class SimpleGenDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        ...
        self.feats = pool.map(self.convert_examples_to_features, \
        [(dic, tokenizer, args) for dic in datas])
...
class SimpleCUPDataset(TextDataset):
    ...
    def convert_examples_to_features(self, item):
        ...
        # code_before,code_after = from_codisum_diff_get_code(diff)
        # inputstr = "<msg> " + old_msg + " " + tokenizer.sep_token + code_before + tokenizer.sep_token + code_after
...
class SimpleReFineDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        self.tokenizer = tokenizer
...
class SimpleJITDPDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1, oversample=False):
        ...
        features_data=features_data[:16000]
        if file_path.endswith('pkl'):
            features_data = features_data[:2000]
        print(features_data)
...
class ReviewExample(object):
    ...
    # def __repr__(self):
    #     return f'idx:{self.idx}, oldf:{self.oldf}, diff:{self.diff}, msg:{self.msg}, cmtid:{self.cmtid}, max_len:{self.max_len}, y:{self.y}, prevlines:{self.prevlines}, afterlines:{self.afterlines}, lines:{self.lines}, labels:{self.labels}, tokenized:{self.tokenized}, avail:{self.avail}, input:{self.input}, lang:{self.lang}, max_tgt_len:{self.max_tgt_len}, tokenizer:{self.tokenizer}'
...
def read_review_pkl_examples(args, dataset, data_num=-1, tokenizer=None, skip_unavail=True):
    """Read examples from filename."""
    examples = []
    idx = 0
